...
function SupplyControl({issuer, onTransactionResult, onTransactionSubmit}: Props) {
    ...
    const submitIncreaseSupply = async () => {
        try {
            ...
            const result = await provider.increaseSupply(issuer.id, issuer.adminAuthResource, formValues.increaseSupply.trim());
    ...
    const submitDecreaseSupply = async () => {
        try {
            ...
            const result = await provider.decreaseSupply(issuer.id, issuer.adminAuthResource, formValues.decreaseSupply.trim());
            onTransactionResult(result);
        } catch (e) {
            console.error(e);
            setError(e);
    ...
    return (
        <StyledPaper sx={{padding: 6}}>
            {error && <Box sx={{paddingBottom: 4}}><Alert severity="error">{error.message || error}</Alert></Box>}
